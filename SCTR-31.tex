...
* Rz = +351 urad
    ...
    \end{minipage}
    &
          \href{https://jira.lsstcorp.org/browse/LVV-18870}{LVV-18870}
    \\\hline
    \href{https://jira.lsstcorp.org/secure/Tests.jspa#/testCase/LVV-T1796}{LVV-T1796}
    & Fail &
    \begin{minipage}[]{9cm}
...
accurately published.}\\[2\baselineskip]{\\
}
...
out successfully with telemetry data in the EFD -~\emph{Should we
specify the telemetry test case case?}
...
\paragraph{Test Case LVV-T1810 - calibrate M2 optimal position and position sensor offset }\mbox{}\\

Open  \href{https://jira.lsstcorp.org/secure/Tests.jspa#/testCase/LVV-T1810}{\textit{ LVV-T1810 } }
test case in Jira.

The objective is to verify that we are able to (1) optimize the mirror
position relative to the cell and redefine the optimal position as
mirror reference position and (2) reset the offsets of the (independent)
position sensors so that they show zeros at the mirror reference
position.\\
The position should be provided by the 6 passive support. The position
from the displacement sensors should be provided for
verification.\\[2\baselineskip]{The objective of this test case is to
partially re-verify the force measurement accuracy and repeatability
requirements defined in \citeds{LTS-146}, after shipment from the vendor's
facility to the Summit facility. This test case will involve optimizing
the mirror position relative to the cell and redefine the optimal
position as a mirror reference position. Once the optimal position has
been determined, the offsets of the independent position sensor will
need to be reset so that they show zero when at this reference position.
The position from the displacement sensors will be provided to show
partial verification of the accuracy and repeatability requirements.}
...
cart.\\[2\baselineskip]{The objective of this test case it to re-verify
the requirements specified in \citeds{LTS-146} regarding the forces of the M2
support system. This test case will first test that the actuators are
applying the correct forces according to the Look-Up Table through the
M2 cart's entire range of motion. This test case should show that the
LUT forces are being updated at 20Hz and safely support the mirror. In
addition, the M2 inclinometer data should show consistency to that of
the M2 cart control and zenith angles displayed on the
GUI.~}\\[2\baselineskip]{Version 1 - No elevation}\\
{Version 2 - Elevated}
...
4 & Description \\
 & \begin{minipage}[t]{15cm}
{\footnotesize
{(From the inclinometer test case)}\\
{data analysis:\\
...
distances of the M2 meet requirements.\\[2\baselineskip]{The objective
of this test case is to re-verify the safety stops are in place as
specified in \citeds{LTS-146} after shipment from the vendor's facility to the
Summit facility. This test will involve inspecting that the safety stops
are in place first and then moving the system until the motion limits
are reached. The results of this test should show that the system enters
the fault state when the motion limits are reached and prevent the M2
mirror from contacting the safety stops.}
...
good repeatability and stability.\\[2\baselineskip]{The objective of
this test case is to re-verify the force measurement, accuracy and
repeatability requirements specified in \citeds{LTS-146}, after shipment from the
vendor's facility to the Summit facility. This test will confirm by
repeating the same set of steps for all six direction that the
...
To verify that the M2 force balance system works as
designed\\[2\baselineskip]{The objective of this test case is to verify
the force balancing requirements of the actuators specified in \citeds{LTS-142},
after shipment from the vendor's facility to the base facility. This
test case will be using the inner loop control during open loop mode as
well as the outer and inner control loops in closed loop
mode.~}\\[2\baselineskip]
...
stiffness\textgreater{}30N/um\\[3\baselineskip]{The objective of this
test case is to verify the actuator stiffness requirements specified in
\citeds{LTS-146}, after being shipped from the vendor's facility to the Summit
facility. The test will involve using a stand alone spare actuator in
order to produce a more accurate spring constant.~~}
...
tangents).\\[2\baselineskip]{The objective of this test case is to
re-verify the force limits of the M2 actuators as specified in \citeds{LTS-146},
after shipment from the vendor's facility to the Summit facility. This
will be done by testing the force limits of one axial actuator and one
tangent actuator. The axial actuator will be shown to be able to output
at least 444N of force and trigger an alarm before reaching 666N.
Similarly, the tangent actuator will be shown to be able to output at
least 6000 N of force and trigger an alarm before reaching 9000N. The
telemetry will be monitored to ensure that the force requirements are
met without going past the force limits.}
...
switches are in place and work properly.\\[2\baselineskip]{The objective
of this test case is to re-verify the tangent link stroke requirement
defined in \citeds{LTS-146}, after shipment from the vendor's facility to the
Summit facility. In addition to verifying the stroke requirement, this
test will be used to determine the location of the software limit
without commanding the actuators to the hard
stops.}\\[2\baselineskip]{\textbf{Note: I noticed that only the tangent
link stroke has a re-verification element traced to this. When we went
through the requirements we would be re-verifying, we only specified the
tangent link stroke requirement saying that we would follow the Harris
test report. Should we create a VE for this?}}
...
\\ \cdashline{2-2}
 ...
 & Test Data \\
 & \begin{minipage}[t]{15cm}{\footnotesize
{Per discussion with Doug, we shouldn't actually test the hard stop
limit as to not damage it. as long as we have software limit, we will be
fine.}
...
at 20Hz.\\[3\baselineskip]{(Delete and combine with AOS duration) Since
the AOS duration is a predecessor and later test case, I think we should
just combine them.}
...
temperatures are with specifications under operational
conditions.\\[2\baselineskip]{The objective of this test case is to
re-verify the Mirror Cell Temperature requirement specified in \citeds{LTS-146},
after shipment from the vendor facility to the Summit facility. To
verify that the mirror cell temperature meets its requirement during
operation, this test will be executed once at the beginning of the day
before any use and once at the end of a day of use. Instead of using all
sixteen temperature sensors, the average difference between the exhaust
temperature and intake temperature will be determined by using only four
sensors (2 exhaust temperature sensors and 2 intake temperature
sensors). This average will show that the cell temperature requirement
has been met.}\\[2\baselineskip]{Version \#1 - initial testing that Bo
did}\\
...
8 & Description \\
 & \begin{minipage}[t]{15cm}
{\footnotesize
Repeat the above at the beginning of a few testing days.
...
Look-Up Table.\\[2\baselineskip]{\emph{Combine with M2 LUT test case} }
...
\end{minipage} \\ \cdashline{2-2}
 ...
 & Issues found executing this step:  \\
 & \begin{minipage}[t]{13cm}{\footnotesize
\href{https://jira.lsstcorp.org/browse/LVV-18870}{LVV-18870}~~Clean up the M2 zenith angle inconsistency
...
vendor's facility to the Summit facility.}\\[2\baselineskip]{\textbf{I
think we should leave this as a separate test case since we need to
verify when the baffle is installed but should this be part of this test
cycle still?}}
...
pads weigh less than 1600 kg\\[3\baselineskip]{The objective of this
test case is to partially re-verify the mirror definition requirement
specified in \citeds{LTS-146}, after shipment from the vendor's facility to the
Summit facility. This test will only involve the use of the surrogate
mirror. The purpose of this test case is to verify the ability of the M2
cell assembly to support an object of similar shape and weight. After
this functionality is confirmed, the requirement will be fully verified
through a test involving the real mirror.~}
...
robustly with active optics forces over extended period of
time.\\[2\baselineskip]{The objective of this test case is to verify the
active optics requirements specified in \citeds{LTS-146} after the shipment from
the vendor's facility to the Summit facility. This includes testing the
M2 control system operates at a bandwidth of at least 20Hz.~}\\
{The test will incorporate testing of the additional offset forces
applied to the mirror support control system during visits of 37s. The
steps of this test will also determine how long the maximum length of a
single csv file can be run.~}\\[3\baselineskip]{\emph{Should the scope
of this test cover control bandwidth and VMS test cases?}}
