...
facility to the Summit facility. This test case will be executed with an
E-stop in place of the interlock system. The results of this test should
show that the M2 support control system behaves as expected, sending the
system into a FAULT state when the E-stop is pressed and cuts all power
from the M2 actuators.~}
...
{The purpose of this test case is to re-verify the interface between the
M2 controller and TCS via SAL 4.0, as specified in \citeds{LTS-146} and \citeds{LTS-162},
after shipment from the vendor's facility to the Summit facility. The
initial telemetry published to the EFD will first be compared to the
information that is displayed on the EUI and on an output binary file
from Matlab. Once the initial telemetry data is seen to be consistent
across the EFD, the EUI and the output binary file, a force will be
added to one of the actuators. }\\[2\baselineskip]The objective of this
test case is to verify the M2's ability to accurately publish telemetry
as specified in \citeds{LTS-162}. In order to do so, the telemetry data being
published to the EFD must first be compared to the information that is
displayed on the EUI and on an output binary file from Matlab. Once the
initial telemetry data is seen to be consistent across the EFD, the EUI
and the output binary file, 20N of force will be loaded onto one of the
actuators. The purpose of this will be to verify that the telemetry data
accurately reports the 20N of added force. If the telemetry is confirmed
to be accurate, we will verify that the telemetry topics specified in
\citeds{LTS-162} are all consistently reported through the EFD, EUI and output
binary files.~
...
{The purpose of this test case is to re-verify the inner loop control
loop requirement, as specified in \citeds{LTS-146}, after shipment from the
vendor's facility to the Summit facility. The test steps are meant to
verify that inner control loop applies motor steps as commanded in open
loop mode.~}{\\
}
...
The objective of this test case is to re-verify both the inner and outer
control loop requirements as defined in \citeds{LTS-146} after shipment from the
vendor's facility to the Summit facility. The test steps are meant to
verify the {operation} of the ~the outer control loop while in closed
loop mode. The outer control loop of the M2 control system takes the
force commands and convert them into displacements, then apply them on
the individual actuators. This test case also verifies the working
mechanism of the force balance system. When the commanded forces give
nonzero total net forces or moments, the force balance system
compensates those by applying pre-determined force distributions.
...
after shipment from the vendor's facility to the Summit facility. Within
closed loop mode, the M2 mirror control system will be verified to be
able to position the mirror with good repeatability and stability.}
...
\paragraph{ LVV-T1829 - M2 position sensors }\mbox{}\\
...
facility to the Summit facility. This test case will be executed before
the M2 reference position and position sensor offset test case because
we will need to first verify that the 12 sensors work according to the
hardpoint encoders.~}\\[2\baselineskip]\textbf{{Version \#1 - verify
position sensors work properly according to the hardpoint encoders. Take
the data to verify the code we came up with to check that the M2
position works properly.~}}\\
\textbf{{Version \#2 - After we have code implemented in the software,
verify it actually works and displays on the GUI as expected with both
sets of positions consistent. (\emph{Once we feel like version 1 is
done, I will create the second version and update it with this
information)}}}

\textbf{ Preconditions}:\\
test case
\href{https://jira.lsstcorp.org/secure/Tests.jspa\#/testCase/LVV-T1791}{M2
position stability and repeatability} has been carried out successfully
with telemetry data in the EFD.
...
{The objective of this test case is to partially re-verify the force
...
cell and redefine the optimal position as a mirror reference position.
...
when at this reference position. The position from the displacement
sensors will be provided to show partial verification of the accuracy
...
active-optics forces requirement will be verified by the M2 actuator
bump test in conjunction with the bending mode test and AOS duration
test. This test case will specifically verify that the M2 actuators are
alive and responding to commands properly while in closed loop mode and
...
can be applied, meeting the accuracy and repeatability requirements
specified in \citeds{LTS-146}. As mentioned in the M2 Actuator Bump Test, this
test will also verify the Active-Optics Forces requirement. This test
will involve applying bending mode forces for about 5s and should show
that the measured forces are consistent to the commanded forces.}
...
{The objective of this test case it to re-verify the requirements
specified in \citeds{LTS-146} regarding the forces of the M2 support system. This
test case will first test that the actuators are applying the correct
forces according to the Look-Up Table through the M2 cart's entire range
of motion. This test case should show that the LUT forces are being
updated at 20Hz and safely support the mirror. In addition, the M2
inclinometer data should show consistency to that of the M2 cart control
and zenith angles displayed on the GUI. }\\[2\baselineskip]{Version 1 -
No elevation}\\
{Version 2 - Elevated}
