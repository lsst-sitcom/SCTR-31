...
\end{minipage} \\ \cdashline{2-2}
 ...
 & Actual Result \\
 & \begin{minipage}[t]{15cm}{\footnotesize
local time 7:21pm. We will go to EFD later to get the forces, which
should be optimized forces at zenith, and compare to calculations based
on Harris csv files\\
Done 3/26/20.
...
\end{minipage} \\ \cdashline{2-2}
 ...
 & Actual Result \\
 & \begin{minipage}[t]{15cm}{\footnotesize
GUI shows balance forces (F\_HC), but we don't have the
Fx/Fy/Fz/Mx/My/Mz as seen by the hard points. We don't have
Fx/Fy/Fz/Mx/My/Mz as output by all actuators.\\
These can be easily calculated using F\_HC and the actuator coordinates.
This can be done by reading out F\_HC from binary files and calculations
done with a simple notebook. We will have to do this later and come back
to this.
...
Te-Wei points out that the M2 control system has z axis going from M1M3
to the sky. While Bo's FEA coordinate system has z going from M2 to
M1M3. So it makes sense now.\\[2\baselineskip]2:58pm local, we are back
to do de-energization. reapplied bm1, 500nm.
...
5 & Description \\
 & \begin{minipage}[t]{15cm}
{\footnotesize
tilt M2 to 30 degrees off zenith, and repeat the above test.
...
{\footnotesize
data analysis:\\
check mirror position and actuator forces (including tangent links)
before and after the de-energization.
...
\end{minipage} \\ \cdashline{2-2}
 ...
 & Actual Result \\
 & \begin{minipage}[t]{15cm}{\footnotesize
Using checkForceLatency.ipynb, we confirmed that 1s is enough time for
the forces to settle. So there is no need to redo the test with longer
time intervals.
