...
Verify the E-stop works correctly.\\
{The purpose of this test case is to verify that the M2 support control
system will behave as expected and cut the power from the actuators when
the E-stop has been hit.}\\
\emph{{The objective depends on how we will be writing the test cases;
In the preliminary phase, this test case was just used to test that
hitting the E-stop would cut the power from the actuators. (This is how
we expect the system to behave during operation with the interlock
system in place of the E-stop) In the formal phase, the purpose of this
test is to verify that hitting the E-stop will send the system into
FAULT state. At the moment, there is no fault state and is on Te-wei's
to do list.}}\\[3\baselineskip]
...
verify M2 SAL4.0 telemetry against EUI display and output binary files\\
{The objective of this test case is to verify that the telemetry and
events being published to the EFD is consistent to the information on
the EUI and output binary files.~}\\
{The following are the supported telemetry:}

\begin{itemize}
\tightlist
\item
  {mirrorPositionMeasure}
\item
  {axialForcesMeasured}
\item
...
{All telemetry and log events will be checked through the use of python
notebook.~}\\[2\baselineskip]{\emph{Is this limit switch status
