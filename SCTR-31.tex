...
\end{minipage} \\ \cdashline{2-2}
 ...
 & Actual Result \\
 & \begin{minipage}[t]{15cm}{\footnotesize
\href{https://jira.lsstcorp.org/secure/Tests.jspa\#/testPlayer/testExecution/LVV-E1051}{LVV-E1051}
...
\end{minipage} \\ \cdashline{2-2}
 ...
 & Actual Result \\
 & \begin{minipage}[t]{15cm}{\footnotesize
\href{https://jira.lsstcorp.org/secure/Tests.jspa\#/testPlayer/testExecution/LVV-E1052}{LVV-E1052}
...
1 & Description \\
 & \begin{minipage}[t]{15cm}
{\footnotesize
with M2 facing up, reconfigure the force force limits (make a copy of
default configuration, do not overwrite):\\
for axial actuators use 300N (1600*9.8/72 = 218N is need to support
mirror weight)\\
for tangent links use 1000N (4526N from each is needed at horizon point,
for zenith they are all be zero)
...
{\footnotesize
in closed-loop mode:\\
loop over all 78 actuators (axial and tangent),\\
increase forces to 0.9*limit for each of them, make sure it is OK.\\
increase forces to 1.15*limit for each of them, see if they are
rejected.
...
{\footnotesize
data analysis:\\
check the response of force balance system for forces = 0.9*limit\\
...
\end{minipage} \\ \cdashline{2-2}
 ...
 & Actual Result \\
 & \begin{minipage}[t]{15cm}{\footnotesize
According to Te-Wei, this can not be done yet. We don't know if the
software limits are implemented or not.
...
4 & Description \\
 & \begin{minipage}[t]{15cm}
{\footnotesize
in open loop, command actuators by mm, approach the strokes
...
-500N. clear the errors.\\
-1mm, 3:56pm. 6055: excessive force detected. force on B15 goes to
+622N. clear the errors. Note that what we put in the GUI is the
relative value.\\
+0.1mm x 3 times went too far\\
-0.05mm\\
...
1 & Description \\
 & \begin{minipage}[t]{15cm}
{\footnotesize
rotate the M2 cart over its full range. Record telemetry during the
process.
...
data analysis:\\
examine telemetry file, to make sure\\
the LUT forces at updated at 20Hz\\
compare the LUT forces to our expectations based on the LUT csv files.
...
4 & Description \\
 & \begin{minipage}[t]{15cm}
{\footnotesize
in EUI, add +20N to B3, then -20N to B3\\
repeat for C8\\
repeat for D17\\
repeat for A2, A4, A6
...
7 & Description \\
 & \begin{minipage}[t]{15cm}
{\footnotesize
data analysis: (hard points are A1,A3,A5,B6,B16,B26)\\
demonstrate that for open-loop, the hard points take the extra forces\\
for closed-loop, all actuators work together to take out the extra
forces
...
force balance system updates forces at 20Hz\\
LUT updates forces at 20Hz
...
(higher temp). Supposedly no ambient measurement yet. Ambient in
requirement is meant for operation conditions anyway.
...
{\footnotesize
data analysis:\\
from the above telemetry data, compare the M2 inclinometer reading with
what the M2 cart gives
...
1 & Description \\
 & \begin{minipage}[t]{15cm}
{\footnotesize
get a thermometer ready for the test duration, so that the ambient
temperature data can also be used for temperature test.\\
...
4 & Description \\
 & \begin{minipage}[t]{15cm}
{\footnotesize
do a 2nd test run for 10 minutes\\
(at 30 degree off zenith)
...
5 & Description \\
 & \begin{minipage}[t]{15cm}
{\footnotesize
do a 3rd test run for 30 minutes (depending on file size constraints)\\
(at 45 degree off zenith)
...
{\footnotesize
data analysis:\\
force stability within each 15s period (5:00-20:00, 22:00-37:00)\\
...
Bo is the only running the test, and needs to work on dome seeing
monitor a bit before the bus leaves today. So, we are not going to take
these independent environmental measurements in 1/2 intervals.\\
...
\end{minipage} \\ \cdashline{2-2}
 ...
 & Actual Result \\
 & \begin{minipage}[t]{15cm}{\footnotesize
14:07 utc 3/6/20. starting tests with duration\_6min.csv\\
