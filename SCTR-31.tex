...
E-stop has been hit.~}\\[2\baselineskip]\emph{{The only requirement we
have traced is the interlocks requirement that says it must be
compatible to the GIS. In the description of that requirement, we noted
that it would be verified by checking if the system goes into the fault
state. However, the test script doesn't include a step in specifically
checking the system is in fault state.~}}
