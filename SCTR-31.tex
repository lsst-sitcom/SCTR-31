...
support control system behaves as expected, sending the system into a
FAULT state when the E-stop is pressed and cutting all power from the M2
...
accurately published.}\\[3\baselineskip]{\emph{Is this limit switch
...
9 & Description \\
 & \begin{minipage}[t]{15cm}
{\footnotesize
{AxialActuatorAbsolutePositionSteps}
...
10 & Description \\
 & \begin{minipage}[t]{15cm}
{\footnotesize
{TangentActuatorAbsolutePositionSteps}
...
11 & Description \\
 & \begin{minipage}[t]{15cm}
{\footnotesize
{AxialActuatorPositionAbsoluteEncoderPositionMeasured}
...
12 & Description \\
 & \begin{minipage}[t]{15cm}
{\footnotesize
{TangentActuatorPositionAbsoluteEncoderPositionMeasured }
...
hardpoint encoders}\\[2\baselineskip]\textbf{{Incorporate item\#12 from
the wishlist. We need to execute this test case before the M2 reference
position and position sensor offset test case. This test case first
verifies that the software for the sensors work correctly.}}\\
\textbf{{Version \#1 - verify position sensors work properly according
to the hardpoint encoders. Take the data to verify the code we came up
with to check that the M2 position works properly.}}\\
...
facility to the Summit facility. This test case will test the
following:}

\begin{enumerate}
\tightlist
\item
  {The ability to optimize the mirror position relative to the cell and
  redefine the optimal position as mirror reference position}
\item
  {Reset the offsets of the (independent) position sensors so that they
  show zeros at th emirror reference position.~}
\end{enumerate}
...
support system. This test case will test that the M2 support system
correctly operates through the M2 cart's entire range of motion. Once
the M2 cart has been rotated over its full range, the data will be
analyzed to confirm the following:}
...
\item
  {The telemetry data of the M2 inclinometer is consistent to the data
  from the M2 cart control and zenith angles displayed on the EUI}
\item
  {The sum of the forces are less than 1600kg}
\end{enumerate}

{Version 1 - No elevation}\\
{Version 2 - Elevated}
...
designed\\[2\baselineskip]\emph{{Combine with predecessor test case}}
...
switches are in place and work properly.\\[3\baselineskip]{The objective
of this test case is to re-verify the stroke requirement defined in
\citeds{LTS-146}, after shipment from the vendor's facility to the Summit
facility. In addition to verifying the stroke requirement, this test be
used to determine the location of the software limit and confirm that
the limit will be displayed on the GUI.}
...
at 20Hz.\\[2\baselineskip]\emph{{Combine with M2 AOS test case?}}
...
after shipment from the vendor facility to the Summit facility. This
test will need to be executed last because it requires the M2 mirror
control system to have been on for at least a couple of days of
testing.}\\[2\baselineskip]{Version \#1 - initial testing that Bo did}\\
...
\\ \cdashline{2-2}
 ...
 & Test Data \\
 & \begin{minipage}[t]{15cm}{\footnotesize
\textbf{Split out into individual steps}
...
its impact on system image quality is within its
allocation.\\[2\baselineskip]\emph{{Combine with M2 AOS test?}}
...
pads weigh less than 1600 kg\\[3\baselineskip]{\emph{Combine with M2 LUT
test case}}
...
the vendor's facility to the Summit facility.~}\\
{\emph{Should the scope of this test cover control bandwidth and VMS
test cases?}}
