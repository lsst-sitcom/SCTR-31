...
support system, the mirror cell, the mirror cell sensors, aperture ring
...
system will behave as expected and transition into fault state once the
E-stop has been hit. Furthermore, the new interlock will turn off the
power relay of the actuators causing the actuators to lose
power.~}\\[2\baselineskip]\emph{{The only requirement we have traced is
the interlocks requirement that says it must be compatible to the GIS.
In the description of that requirement, we noted that it would be
verified by checking if the system goes into the fault state. However,
the test script doesn't include a step in specifically checking the
